...
import eu.opensourceprojects.mondo.benchmarks.transformationzoo.ZooBenchmarksPlugin;
import eu.opensourceprojects.mondo.benchmarks.transformationzoo.util.FileUtils;
...
public abstract class TestNonRegression extends TestCase {
	...
	public void testNonRegression() throws Exception {
		...
		if (CoreService.isEclipseRunning()) {
			this.baseDirectory = FileLocator.getBundleFile(ZooBenchmarksPlugin.getDefault().getBundle());
		} else {
			this.baseDirectory = new File(ZooBenchmarksPlugin.class.getResource("").toURI()).getParentFile() //$NON-NLS-1$
