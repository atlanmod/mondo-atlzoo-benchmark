...
import eu.opensourceprojects.mondo.benchmarks.transformationzoo.instantiator.SpecimenGenerator;
...
protected int[] modelsSize = {1000}; // this size includes model objects and properties
	...
	private String computeRootElement() throws GenerationException {
		...
		try {
				...
				while (iter.hasNext()) {
					...
					if (((EClass)eObject).getEAllSuperTypes().isEmpty() &&
							((EClass)eObject).getEAllContainments().isEmpty()) {
						isRoot = true;
						break;
					}
	...
	public void runGeneration() throws GenerationException {
		...
		for (int i = 0; i<loop; i++ ) {
			...
			for (int j = 0; j < setSize[i]; j++) {
				...
				genModelConf.setNumberOfProperties(
						inferNumberOfProperties(numberOfElements));// base * (scaleBase)^tour
	...
	private void saveModel(int maxElements, int index, List<EObject> generatedObjects) throws GenerationException {
		Resource resource = new ResourceSetImpl()
				.createResource(formatURI(getMetaModelName(), maxElements, index));
		System.gc();
